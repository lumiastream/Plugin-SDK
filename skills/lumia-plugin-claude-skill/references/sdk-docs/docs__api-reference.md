...
- **`refreshTtsVoices(): Promise<void>`** – ask Lumia to re-pull your `ttsVoices()` list. Call it after the user sets an API key or your available voices change, so the native TTS voice picker updates without a plugin reload. Pair it with `"hasTtsVoices": true` in your manifest `config` (see the [Manifest Guide](./manifest-guide.md)).
