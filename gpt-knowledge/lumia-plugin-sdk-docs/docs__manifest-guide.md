...
That single call drives the `{{heart_rate}}` overlay variable, the Pulse heart-rate zone alerts, calorie tracking, and Studio heart-rate light reactions — the same pipeline the native heart-rate integrations use. See `updateHeartRate` in the [API Reference](./api-reference.md).
...
See `ttsVoices`, `synthesizeTts`, and `refreshTtsVoices` in the [API Reference](./api-reference.md).
...
For all field types, properties, validation options, and examples see [Field Types Reference](./field-types-reference.md).
...
For full property lists and examples see [Field Types Reference](./field-types-reference.md).
