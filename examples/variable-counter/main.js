...
class VariableCounter extends Plugin {
  async onload() {
    ...
    await this.lumia.addLog(`[variable-counter] Counter initialised at ${startValue}`);
  }
  ...
  async actions(config = {}) {
    ...
    for (const action of actions) {
      if (action.type === 'increment') {
        await this.increment(Number(action.data?.amount));
      }
  ...
  async increment(amount) {
    ...
    const current = Number(this.lumia.getVariable('variable-counter_count')) || 0;
  ...
  async reset() {
    const startValue = Number(this.settings.startValue) || 0;
    await this.lumia.setVariable('variable-counter_count', startValue);
    await this.lumia.addLog(`[variable-counter] Counter reset to ${startValue}`);
  }
}
