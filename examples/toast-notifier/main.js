...
class ToastNotifier extends Plugin {
  ...
  async actions(config = {}) {
    ...
    for (const action of actions) {
      ...
      const message = action.data?.message || this.defaultMessage;
      const duration = Number.isFinite(Number(action.data?.duration))
        ? Number(action.data.duration)
        : this.defaultDuration;

      await this.lumia.showToast({ message, time: duration });
      await this.lumia.addLog(`[toast-notifier] Toast displayed: "${message}" (${duration}ms)`);
    }
  }
}
