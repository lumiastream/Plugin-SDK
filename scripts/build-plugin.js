...
const fs = require('fs');
const { readManifest, validateManifest, collectFiles, createPluginArchive } = require('./utils');
...
Usage: npm run build:plugin -- [options]
...
async function main() {
  ...
  try {
    ...
    if (!files.length) {
      console.error('✖ No files found to package (did you point to the plugin root?)');
      process.exit(1);
    }

    const filename = `${manifest.id}-${manifest.version}.lumiaplugin`;
    const outputPath = options.outFile ? options.outFile : path.resolve(filename);
